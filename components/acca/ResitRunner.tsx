...
// SHARED UI, ONE PROP (2026-08-08). Moved out of app/acca/resit/page.tsx so both papers
// render the same component: app/acca/resit (APM) and app/acca/afm/resit (AFM) differ ONLY
// in `paper` — the same split app/acca/mock and app/acca/afm/mock already use for SitRunner.
// Every question, label and CTA below is driven by that prop.
...
// AFM's published corpus has no section C or D drills (M&A, reorganisation and restructuring)
// and no browsable section A, so those areas are ABSENT from the topic list rather than
...
export default function ResitRunner({ paper }: { paper: AccaPaper }) {
  ...
  function inputsBody() {
    return {
      // Always sent. The route refuses an absent paper rather than defaulting to APM,
      // because AFM and APM lo_code prefixes collide exactly.
      paper,
      score: scoreNum,
      sitting: `${month} ${year}`,
    ...
    <div className="resit">
      ...
      {step < 4 && (
        <div className="resit-head">
          <p className="resit-kicker">Free ACCA {paper} resit diagnostic</p>
      ...
      {step === 1 && (
        <div className="resit-card">
          <h2 className="resit-card-title">Your last {paper} attempt</h2>
          ...
          <label className="resit-field">
            <span className="resit-label">How many times have you sat {paper}?</span>
      ...
      {step === 2 && (
        <div className="resit-card">
          ...
          {/* Stated, not implied. An area Gradd cannot serve is left off the list above, so
              without this the omission would read as "nothing to fix there". */}
          {coverageNote && <p className="resit-coverage">{coverageNote}</p>}
      ...
      {step === 4 && (
        <div className="resit-plan">
          <div className="resit-plan-head">
            <p className="resit-kicker">Your {paper} resit plan</p>
          ...
          {/* CTA into the free drills — thread the diagnosed weakest area into ?next= so the
              ...
              paper= IS LOAD-BEARING, not decoration: without it the drill serve falls back on
...
.resit-coverage {
  ...
  background: rgba(0,0,0,0.03); border: 1px solid var(--line, rgba(0,0,0,0.08));
  font-size: 13px; line-height: 1.55; color: var(--muted, #5b5b5b);
}
