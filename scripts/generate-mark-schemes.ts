...
- These are NOT interchangeable. A point that attributes MSB > MPB to a production externality, or MSC > MPC to a consumption externality, is wrong and must not appear in accepted_points.`
    : spec.subject === 'IB_BUSINESS_MANAGEMENT' ? `

CONCEPTUAL MAPPING GUARDRAILS — IB BUSINESS MANAGEMENT CONTENT QUESTIONS:

Sources of finance — internal vs external classification:
- INTERNAL sources = funds generated from within the business itself: retained profit (net profit kept after dividends), sale of assets (disposing of owned property or equipment), reduction in working capital (tightening debtor collection, reducing stock levels).
- EXTERNAL sources = funds obtained from outside the business entity: bank loans, overdrafts, share capital (new share issues), debentures, venture capital, government grants, trade credit, leasing, and — critically — owner's personal savings or personal investment. A sole trader or partner injecting personal money into the business is making an EXTERNAL contribution, not an internal one.
- Do NOT classify owner's personal savings, personal loans, or personal investment as internal sources. The injection comes from outside the business entity — it is EXTERNAL regardless of who provides it.
- The test: internal = generated by the business's own trading activity; external = brought in from any outside source, including the owner personally.` : '';
