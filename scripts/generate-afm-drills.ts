...
async function main() {
  ...
  if (npvBatch) {
    ...
    const plan: { kind: InternationalKind; lo: LoCode; region: string; sector: string }[] = [
      ...
      { kind: 'multinational_dividend_capacity', lo: 'A6a', region: 'Vietnam', sector: 'an Australian (AUD, ~30% corporate) parent drawing dividends from a Vietnamese (VND, 20% corporate) electronics-manufacturing subsidiary — Vietnam levies 0% withholding on outbound dividends to a corporate parent, so the ~10-point differential is a CHARGED additional home tax (remittance timing + differential credit)' },
