...
The conversational tutor (`/api/acca/tutor`, all papers) has an adversarial regression suite: `scripts/redteam-probes.ts` (45-probe matrix × 14 classes — concept/invented-figures, wrong-drill, partial, gibberish, answer-extraction incl. "verify this: <pasted answer>", prompt-injection, regurgitation, right/wrong-method, currency-scale, hint-fishing, emotional, persona-boundary, free-cap/burn edges, long-conversation drift), `scripts/redteam-tutor.ts` (driver — minted free/paid sessions, real route, machine auto-checks: figure-leak / invented-range / cutoff / CTA / call_type / unearned-reveal), `scripts/redteam-judge.ts` (reviewer-model pass → FLAGGED-ONLY). **STANDING RULES:** (1) **re-run the battery after ANY tutor prompt / persona / leg change** — it is a regression gate, not a one-off; (2) **run the judge's `--prod-sample` when 25 GENUINE STUDENT TURNS have accrued since the last run** — real student behaviour is the probe source no manufactured matrix invents (the matrix is the floor, production is the well). Prod firing is guarded (`--target prod --yes-production`); default target is local. Cost ≈ USD 2–6 per full prod run (haiku-heavy).

**⛔ RULE (2) WAS "WEEKLY" UNTIL 2026-09-04, AND IT WAS ASPIRATIONAL IN BOTH HALVES. Corrected, not softened:**

- **It never produced an artefact.** ~7 weeks from the table's creation (2026-07-11) to 2026-09-01: no `prod-sample-*` file on disk, none tracked, none in git history. It was written as a standing weekly rule in THREE documents and ran, as far as any evidence shows, never. **A run that writes nothing is indistinguishable from a run that did not happen** — the same silent-absence-reads-as-health shape as `P-G1`, at the process level rather than in a gate.
- **Its pairing was broken before first use.** It sorted `acca_drill_messages` by `created_at` and scanned backwards for the nearest student row. But the tutor route writes both rows of a turn in ONE `insert([user, assistant])`, so both take the same `now()`: all 1063 historical pairs share an identical timestamp (verified — 1063 groups, exactly 2 rows, both roles, zero exceptions), and a tie has no defined order. **The error rate is an artefact of the tie-break, not a property of the algorithm** — measured over the same 2126 rows: **51%** ordering ties by `id`, **2%** in fetch order, **100%** ordering by `role` (`'assistant' < 'user'`). Quoting one figure would be false precision; the honest statement is that the pairing was UNDEFINED.
- **The mispairing skewed toward FALSE FLAGS**, which is why it would not have announced itself. A correct reply judged against the PREVIOUS turn's question reads as answering the wrong thing, leaking an unrelated figure, or revealing unprompted — all rubric hits. **A clean report would have been the surprising outcome.**

**WHY THE TRIGGER IS TRAFFIC, NOT A CALENDAR (and why N = 25).** The thing being sampled is genuine student turns, and there are ~26 of them in ten weeks. A weekly run judges two turns or none, and its output — nothing — is unreadable as either health or breakage. 25 is **one student's complete engagement at the depth actually observed**: `dd786100` produced exactly 25 turns across 6 drills between 2026-07-18 and 2026-08-20, so the unit of review is a student's journey rather than an arbitrary week. It is one cheap bounded run (25 Sonnet calls against a cached rubric), it fires roughly quarterly at today's real rate — which is the honest cadence — and it **scales itself**: at ten times the traffic it fires weekly with no rule change. ⚠️ **The count is of GENUINE turns, after the `NOT_A_STUDENT` filter** — the harness holds 962 of 1063 turns (90.5%), so an unfiltered threshold is met by the red-team's own traffic inside a single session and the "sample of production" becomes a sample of the matrix.
