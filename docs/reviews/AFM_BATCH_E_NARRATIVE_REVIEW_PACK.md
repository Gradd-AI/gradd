...
**FR1 (GPT-requested, applied 2026-07-24):** the golden GOOD's translation-priority sentence read *"translation exposure warrants little more than awareness"* — technically consistent with the reporting-only anchor, but risked teaching translation exposure as negligible rather than merely lower-priority. Reworded to *"translation exposure is a lower priority because it doesn’t touch cash — though it still matters if a weaker shilling drags on reported gearing or trips a loan covenant"*, keeping the committed priority (hedge transaction first) intact. Full-row sweep of `full_reveal` + `hint` for the same dismissive class found **one further hit** in `full_reveal` ("manage translation lightly by matching-currency funding"), fixed the same way ("treat translation as a lower priority — matching-currency funding is the usual response, but it still deserves attention if it would drag on reported gearing or trip a loan covenant"); `hint` was clean (zero hits). **Re-gated on the REAL grader**: N1 + N4 Rule-23 both re-ran PASS post-edit — the softened GOOD still lands in-band, the BAD still scores below with [F1, F5, F4] raised. P4/P7 + deterministic detectors green. Figures/structure otherwise unchanged.
...
| id | part | required_point (summary) | anchors | disqualifiers | evidence_anchor |
|---|---|---|---|---|---|
| c1 | (i) | identify + DESCRIBE transaction exposure (euro receipts, rand value moves before payment) — not merely name it | f_euro | F1, **F2**, F5 | F9 J16 p.5 **[tier=fundamentals (F9/FM) — level-agnostic failure mode (named-not-described), NOT AFM-examiner-specific]**: "fewer … able to offer clear descriptions" — naming ≠ describing (F2); T8 transaction def |
| c2 | (i) | identify + describe translation exposure (Kenya sub consolidated) AND distinguish it as a reporting-only effect, not a cash-flow threat | f_kenya, f_reported | F1, **F2**, F5 | **HOUSE-AUTHORED** translation≠transaction confusion, caught via the reporting-only anchor; T8 "does not affect day-to-day cash flows"; + F9 J16 p.5 **[tier=fundamentals (F9/FM), level-agnostic]** |
| c3 | (i) | identify + describe economic exposure (competitive position selling to European chains if rand strengthens) | f_european | F1, **F2**, F5 | F9 J16 p.5 **[tier=fundamentals (F9/FM) — level-agnostic failure mode (named-not-described), NOT AFM-examiner-specific]**: economic risk "particularly" poorly described; T8 "competitive strength of imports and exports" |
