...
**Status: BATCH CLOSED 2026-07-21 — LIVE + WALKED.** D1–D5 generated → co-founder rubric review (FR2: D2 1 BLOCKING IFRS16 + 2 minor FIXED; D3 2 prose accepts FIXED) → blind GPT round 1 (FR3: D5 2 MUST-FIX FIXED; D3 1 polish FIXED; D2 1 minor sweep VERIFIED CLEAN) → all re-gated N1–N5 clean → Grant ruled FLIP (published AFM 41→46) → **student walk** (D1 full loop incl. a golden-BAD-shaped attempt + a push-back probe; D5 compressed) — **FROZEN LAYER PASS across both**: scenarios, rubric-tracked resolution, FR-patched reveals served verbatim, correct level tags, zero computed-figure issues (n/a class, conceptual-only holds). D1 close-out patch (internal "GIVEN" convention leak in `context_text`, fixed, re-gated N2/N3 clean). **Conversational-layer findings are SYSTEMIC (not narrative-specific)** — scoped to a dedicated persona-hardening session, tracked in `AFM_SURFACED.md`: a false-positive VaR diagnosis on a CORRECT statement (worst class to date), fog-retraction without ownership, an initial F4 (fence-sitting) miss vs. the stored hint, and invented-inventory sighting #3 (D1 outro). **B-section-live tier: CONTENT COMPLETE (46 published, 11 calculators + pipeline #2) AND WALKED — this claim stands.** **Ad spend is Grant-ruled (21/07): conditioned on the persona-hardening slot shipping + verifying first** — tier-content-complete does not unlock ad spend on its own; the December-sitting-window timeline is unaffected.
...
- **GATES:** N2 ✓ · N3 ✓ · N5 ✓ · N4-pre ✓ · N1 ✓ · **N4 (Rule-23) ✓** — golden GOOD in band, golden BAD below + raised [F1, F5, F4]. **Re-gated N2/N3 clean post-close-out patch**, see below.

**⚠ Close-out finding (2026-07-21, student walk) — internal convention leak, fixed:** `context_text` read "The simulation produced the following **GIVEN** output" — "GIVEN" is generator/doctrine vocabulary (the conceptual-only rule: every narrative-scenario figure is a GIVEN driver, never computed), never meant to reach the student-facing scenario. **Fix:** "the following GIVEN output" → "the following output" — the ONLY "GIVEN" occurrence in the entire row (full-row grep proved zero elsewhere: question/model_answer/hint/full_reveal/answer_schema all clean before AND after). Byte-identical elsewhere — nothing else in `context_text` moved. Anchors unaffected (no scenario_fact key touched), so only **N2 (scenario-anchor) + N3 (generic/copy)** needed re-checking, not the full N1/N4 grader battery — both PASS. Patch script: `scripts/_patch_afm_narrative_d1_leak.ts` (gitignored). Row stays `approved`/`published=true` throughout — a live prose fix, not a status change.

### context_text (scenario — the Monte Carlo output is GIVEN)
> Cảng Sao Biển JSC ("CSB") is a Vietnamese state-linked port operator seeking board approval to expand its deep-water container terminal at Vũng Tàu. The expansion project requires an upfront capital commitment of USD 420 million. Because throughput demand, construction cost overruns and terminal-gate tariffs are all highly uncertain, CSB's financial advisers ran a Monte Carlo simulation with 10,000 iterations across those three key variables. The simulation produced the following output: a mean (expected) NPV of +USD 38 million, a standard deviation of NPV of USD 61 million, a probability of a negative NPV of 27%, and a project Value-at-Risk (VaR) of USD 55 million at the 95% confidence level. The board must decide at its next meeting whether the project's risk profile is acceptable before committing capital.
...
- **STUDENT WALK CLOSED (2026-07-21) — BATCH FULLY CLOSED.** D1 walked full loop (incl. a golden-BAD-shaped attempt + a push-back probe); D5 walked compressed. **FROZEN LAYER — PASS across both drills:** scenarios rendered correctly, rubric-tracked resolution held, the FR2/FR3-patched reveals served VERBATIM (zero drift from the patched rows), correct intellectual-level tags, and — as expected for a conceptual-only family — zero computed-figure issues (n/a class; there is no figure to leak or miscompute). **D1 close-out patch:** an internal generator-convention word ("GIVEN") had leaked into the served `context_text` — fixed (`scripts/_patch_afm_narrative_d1_leak.ts`, gitignored), re-gated N2/N3 clean, row stayed `approved`/`published=true` throughout (prose fix, not a status change). **Conversational-layer findings (systemic, not narrative-pipeline-specific — journalled to `AFM_SURFACED.md`'s persona-hardening slot, now 7 entries):** a false-positive VaR diagnosis flagging a CORRECT student statement as wrong (the worst-class sighting to date), fog-retraction without explicit ownership when corrected, an initial F4/fence-sitting miss against the drill's own stored hint, and invented-inventory sighting #3 (a D1 outro named a follow-up drill that isn't in inventory). These are ALL conversational-leg/persona-prompt issues, not narrative-marking-pipeline defects — the marker/gates/rubric held throughout.

**B-SECTION-LIVE TIER: CONTENT COMPLETE (46 published, 11 calculators + pipeline #2's 5 narrative drills) AND WALKED — this claim stands.** **AD SPEND RULING (Grant, 21/07/2026): conditioned on the persona-hardening slot (`AFM_SURFACED.md`, 7 categories) shipping + verifying before any spend** — tier-content-complete alone does not unlock it. The December-sitting-window timeline is unaffected.

**NEXT:** persona-hardening session (scoped: rubric/key-facts-in-context retrofit for ALL conversational legs, narrative-first, numeric-family retrofit to follow) — see `AFM_SURFACED.md`. Separately: Grant's tier-claim/ads-trigger ruling.
