...
**Status: D1–D5 GENERATED + inserted (candidate). Co-founder rubric review ADJUDICATED (FR2, 2026-07-21): D1/D4/D5 clean passes; D2 1 BLOCKING (IFRS 16) + 2 minor FIXED; D3 2 prose accepts FIXED. All re-gated N1–N5 clean. Awaiting the blind GPT adversarial round.**
...
2. **KES 8.4 billion → KES 26 billion** across every surface (context, question, model_answer, all affected criteria, scenario_facts, golden_bad) — more realistic sizing for a 120 MW solar-plus-storage build. Full-row grep proved zero "8.4" residue post-patch (FR3 lesson applied).
...
Only `model_answer` changed (rubric/context/golden-BAD untouched — the fixes are prose-only). Patch + re-gate script: `scripts/_patch_afm_narrative_fr2.ts` (gitignored). **N1–N5 re-run clean, all PASS** (real grader).
...
| c4 | (iv) | B3i | 3 | Agency: Jensen FCF-discipline is a genuine benefit for a cash-generative miner, but dominated at 95% by asset-substitution/underinvestment via the 80% covenant → reject/scale back | f_de_target, f_covenant | F1, **F4**, F5 |
...
- **approved-unpublished (all AFM): 0.** No publish flip performed — the cluster stays `candidate` pending the blind adversarial review.
