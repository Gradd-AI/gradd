...
**Calculator #10: international investment & financing (`lib/acca/international.ts`). 4 drills, `status=candidate`, `published=false`, `paper_code=AFM`. AT THE REVIEW GATE (Fix Rounds 1 + 2 + 3 + 4 applied; GPT round 1 = 2 accepts / 0 rejects). CURRENT STATE — regenerated after every fix round.**
