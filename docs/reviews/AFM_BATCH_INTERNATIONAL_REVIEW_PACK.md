...
**Calculator #10: international investment & financing (`lib/acca/international.ts`). 4 drills, `paper_code=AFM`. FLIPPED LIVE 2026-07-18 (GATE-P, Grant-ruled) — `status=approved`, `published=true` for all 4 (`499357f7`/`fcf14ae8`/`eac98c43` B5 browsable + `2b0513a0` A6a/K4 direct-link-only). Fix Rounds 1 + 2 + 3 + 4 applied; GPT round 1 = 2 accepts / 0 rejects. Batch #10 CLOSED pending student walk. Published AFM = 37; parked A3a `47c9d5ce` stays candidate.**

**Fix Round 4 (2026-07-18, GPT round-1 adjudication — both findings ACCEPTED; prose-only, figures byte-identical):** (1) K3 restricted-remittance Step 2 now opens with **"Total foreign FCFF = CNY 166.2m; 70% immediately remittable, 30% blocked."** and the first table column is renamed **"Free portion before WHT (70% of FCFF)"** — the table shows only the remittable slice, so the bare "Foreign cash flow" header understated the total; fixed in the FAMILY builder (`buildIntlRemittanceModelAnswer`), so every future restricted-remittance drill inherits the honest label. (2) K4 context "favourable gap" clause replaced with the reviewer's wording — the differential is on **"the remitted share of foreign taxable profit — an additional tax drag that reduces the AUD cash available for group dividends."** Guarded byte-identical on every graded figure + every table data row (the FR3 refuse-to-write mechanism); `test:international` green.
...
Total foreign FCFF = CNY 166.2m; 70% immediately remittable, 30% blocked.

| Year | Free portion before WHT (70% of FCFF) | Free & remitted net (CNY) | Spot | Home cash flow | PV |
|------|------|------|------|------|------|
