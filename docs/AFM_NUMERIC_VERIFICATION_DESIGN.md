...
3. **Schema storage** — a new `answer_schema jsonb` column on `acca_drills` (nullable; null for APM + discursive AFM) vs a child table. Leaning jsonb (mirrors `per_skill` on `acca_case_marking`). Decide with the scoping migration.
4. **`mode` persistence** — the drill row needs to carry its LO `mode` (or the per-drill numeric flag) so serve/verify can route without re-deriving from the framework. Fold into the scoping migration.
5. Tolerance calibration is a **human QA gate**, like `marks_guide` — budget for per-component review, not model-set tolerances.
