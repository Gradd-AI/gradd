...
**ROOT CAUSE:** Both `student_progress` and `sessions` carry a `lesson_code`. RESOLVED 04/06/2026: `student_progress.current_lesson_code` IS authoritative — session/start reads it (line 54), derives everything from it, and writes it into the new `sessions` row as a snapshot. `sessions.lesson_code` is a snapshot copy, not a competing authority. The earlier "wrong lesson" symptom was a resumed OPEN session (which does not re-run session/start) plus the stale denormalised `current_lesson_name` (below). Fix protocol when manually repointing: close open sessions, set `current_lesson_code` AND `current_lesson_name` together, start fresh.
...
**ISSUE:** [IB] BM subject-key mismatch fixed at RPC layer (interim) — BM serving now lesson-matched
**SYMPTOM:** (See prior entry: BM questions served via random tier-4 fallback because `lessons` uses `IB_BUSINESS` while the RPC filtered on `IB_BUSINESS_MANAGEMENT`.)
**ROOT CAUSE:** Subject-key inconsistency across tables (Rule 30).
**FIX:** Option A applied 04/06/2026 — `fetch_exam_questions_tiered` now translates p_subject to `IB_BUSINESS` for the lessons-table lookups only (`current_unit` and `unit_lessons` CTEs), and additionally filters those lookups by subject (previously unfiltered — safer with three products sharing the lessons table). Verified: BM now returns tier-1 lesson-matched questions with schemes; Econ unchanged (tier-1, schemes intact). Done in Supabase SQL Editor, in a BEGIN/COMMIT with DROP+CREATE (return-type unchanged this time but kept transactional). Rollback = re-run the pre-A function def. The function carries an inline comment marking the translation as temporary.
**OUTSTANDING:** This is a translation, not a normalisation — the inconsistency persists in the data. See the dedicated B-project entry below.
**CATEGORY:** Signals
**SEVERITY:** High

---

**ISSUE:** [IB/LC] Subject-key normalisation across all products — scoped, NOT done (B-project)
**SYMPTOM:** Three products share the `lessons` table with inconsistent subject keys: IB BM = `IB_BUSINESS` (lessons, student_progress) vs `IB_BUSINESS_MANAGEMENT` (questions, mark_schemes); IB Econ = `IB_ECONOMICS` (consistent); and 279 LC Business lessons sit with `subject = NULL` (bare codes like `1.1.1`, `UNIT_1`, Irish-law content — the gradd.ie/Aoife product). App code uses `IB_BUSINESS` as the internal product short-key across ~25 files; the serve-time routes translate to `IB_BUSINESS_MANAGEMENT` at the RPC boundary.
**ROOT CAUSE:** Tables seeded at different times by different scripts with no enforced canonical key; LC lessons never got a subject tag.
**FIX:** None yet — deferred as its own focused session. Recommended canonical value `IB_BUSINESS_MANAGEMENT`. Sequence when done: (1) decide subject value for the 279 NULL LC lessons FIRST (cannot normalise around NULL); (2) data migration on `lessons` + `student_progress` (transactional, idempotent, verified); (3) update ~25 code sites — distinguish DB-subject-value uses (change) from app product-key uses (leave); (4) simplify the RPC (remove the Option-A translation); (5) full regression of billing/auth/onboarding/serving/progress for BOTH IB subjects AND LC. High blast radius — touches billing (checkout, webhooks) and auth (onboarding) paths.
**PREVENTION:** Rule 30. Establish one canonical key per entity at project setup; never let a shared table carry NULL or divergent keys per product.
**CATEGORY:** Signals
**SEVERITY:** High

---

**ISSUE:** [IB] BM Layer 2 content_checklist marking live — verbatim compression on long describe points (minor)
**SYMPTOM:** With the BM scheme-aware marking + verbatim instruction merged, Mia marks a served BM Section-A question against its content_checklist: she anchors to the scheme, counts points correctly (1 of 4 on the test), and names all missing points accurately. But on a `describe` question whose accepted points are full sentences, she compresses them to short labels rather than reproducing the scheme text verbatim (e.g. "limited liability — personal protection" vs the scheme's full sentence). She also did not always state the mark as an explicit "X out of N".
**ROOT CAUSE:** Haiku compresses long accepted-point text even under a verbatim instruction; the BM instruction lands slightly softer than the Econ one. Validated 04/06/2026 on served IB_BM_004 "Describe four characteristics of a private limited company" (4-mark, content_checklist), production/main, fresh session.
**FIX:** Accepted as-is — marking is scheme-anchored, accurate, and gives correct student guidance; verbatim full-sentence quoting is cosmetic for describe-type points and not worth pre-launch polish. Logged as minor. If revisited: strengthen the BM MARK SCHEME instruction to match Econ's force and require an explicit mark, and/or route the mark-award to Sonnet for long-point questions.
**CATEGORY:** Signals
**SEVERITY:** Low

---

**ISSUE:** [IB] Hybrid (calculation) mark schemes wired into live marking — Layer 2 now complete across all served scheme types
**SYMPTOM:** Coverage check 04/06/2026 found content_checklist coverage strong on its target types (~90% of short-answer: state/explain/define/analyse/distinguish) and correctly absent on essay terms (evaluate/discuss/recommend — band-marked) — but `calculate` was 0% schemed (29 Econ + 5 BM calc questions marked from memory, the riskiest gap). Calc questions need hybrid (method_marks + answer_marks), which existed but was never wired into serving.
**ROOT CAUSE:** Two parts. (1) The RPC's LEFT JOIN LATERAL pulled only `scheme_type='content_checklist'`. (2) For the only two served calc-orphans (no seed hybrid), a pass-verified hybrid scheme existed but sat in `status='rejected'` — generated and verifier-passed in a prior session, never promoted in admin review (approved_by null, no human rejection). The "~29 rejected hybrids" scare number was mostly superseded duplicates; only 2 served questions actually lacked a working scheme.
**FIX:** (a) Promoted the 2 pass-verified hybrid schemes to seed (targeted UPDATE, verification_status='pass' guard). (b) RPC extended to pull content_checklist OR hybrid (lateral join, prefers content_checklist, returns scheme_type). (c) system-prompt.ts formatter branches on scheme shape (accepted_points vs method_marks + ANSWER line). (d) Econ marking step gained a METHOD MARKS clause (award each step, error-carried-forward, ANSWER mark conditional, missed steps verbatim). (e) Scheme-bearing sessions routed to Sonnet (Rule 31). All merged, validated live on served IB_ECON_152 comparative-advantage calc: Mia opened "Mark: 2 out of 4", reasoned cleanly, quoted the scheme's missing gains-from-trade method verbatim.
...
## 13 June 2026 — Lessons from navigation + review build

### Signal stripping for any read-back surface

**ISSUE:** `session_messages.content` stores the raw unstripped model stream. Displaying stored transcript content without running `stripSignals()` exposes all machine-facing signal tokens to students.
**SYMPTOM:** `[SESSION_OPEN] Begin the session now. Teach IB_BM_029 from the start.` appearing as the first chat bubble in a student's transcript; WEAK_AREA_FLAG JSON visible in the conversation history.
**ROOT CAUSE:** Signal tokens were stripped at the prompt-generation layer (before sending to the model) but not at the read-back layer. The model emits signals mid-stream as continuations (WEAK_AREA_FLAG, LESSON_COMPLETE, etc.) — these are stored raw in `session_messages.content`. Strip logic existed only on the way in, never on the way out from the DB.
**FIX:** `stripSignals()` in `lib/signal-parser.ts`. JSON-bearing signals (WEAK_AREA_FLAG, TEACH_BACK) stripped via brace-counting to handle `}` inside string values; pipe-delimited tokens via regex; BURN_WALL; SESSION_OPEN (strips token + rest of line). 19 tests including adversarial cases (DIAGRAM_DYNAMIC prefix-collision, brace-inside-string, idempotency). Applied in `TranscriptRenderer` on BOTH roles before display.
**PREVENTION:** Any surface that reads back `session_messages.content` MUST call `stripSignals()`. Strip both roles — signals appear in user turns too (`SESSION_OPEN` is a user-turn bootstrap token, not an assistant-turn token). Pipeline order is load-bearing: `parseDiagramSignal` FIRST (extract diagram code to re-render), THEN `stripSignals`. Reversing strips the diagram token before you can render it.
**CATEGORY:** Signal pipeline
**SEVERITY:** High — student-visible raw tokens in transcript UI

---

### Cache keys must be stable identifiers, not freeform model output

**ISSUE:** The diagram cache was keyed on the model's freeform textual description of a diagram concept.
**SYMPTOM:** Cache hit rate near zero. The model described the same concept with varied wording each call — "Maslow's hierarchy of needs" vs "Maslow hierarchy of motivation needs" — so every call missed and re-rendered from scratch.
**ROOT CAUSE:** Freeform text is LLM output. LLMs vary phrasing across calls even for identical concepts. Any cache key derived from model output is functionally random.
**FIX:** Hand-built library components with deterministic codes (`BM_MOTIVATION_MASLOW`, `BM_MOTIVATION_HERZBERG`, `BM_MOTIVATION_TAYLOR`) registered in `DIAGRAM_MAP`. Named by a pre-defined key, never re-derived from description.
**PREVENTION:** Never key a cache on freeform model output. Stable keys are pre-defined identifiers: lesson codes, diagram component codes, explicit enum values. The dynamic path (`DIAGRAM_DYNAMIC`) is acceptable for long-tail one-off diagrams. For recurring named concepts (motivation theories, Porter, 7Ps, Ansoff, etc.), build a library component with a fixed code — the description-keyed path will never hit consistently.
**CATEGORY:** Performance / caching
**SEVERITY:** Medium — cost + latency overhead; invisible to student in happy path

---

### See-don't-infer on rendered UI — non-negotiable

**ISSUE:** Two separate UI defects in this build cycle passed code review and verbal "PASS" verdict; both were caught only by first actual screenshot.
**SYMPTOM:** (1) Library-wide green contrast: `--brand` CSS variable resolved to `#0e2b1e` (near-black, ~1.2:1 contrast on the dark chat surface). All 61 diagram components had near-invisible green text and elements. Passed "the CSS value for BRAND is set to a green" review. (2) Course picker hierarchy: Browse section was positioned above Continue in the student dashboard — the secondary action appeared dominant over the primary CTA. Passed "the hierarchy in the code is correct" verbal review. Both caught immediately on screenshot.
**ROOT CAUSE:** Reading source (CSS values, JSX structure) is not the same as reading what renders. Contrast, z-index, layout hierarchy, spacing, and colour all depend on the full resolved CSS cascade and browser rendering — none of which is visible in source form.
**FIX:** Screenshot-before-approve protocol established. No UI change merged without a rendered screenshot of the affected surface.
**PREVENTION:** UI changes are approved only after viewing an actual rendered screenshot — not a code review, not a verbal description, not "the values look right." Non-negotiable. Applies especially to: colour contrast (the cascade can resolve a variable to an unexpected value), layout hierarchy (structural order ≠ visual order), hover/active/pressed states, and anything involving CSS custom properties that are re-defined at multiple scopes.
**CATEGORY:** Process / review discipline
**SEVERITY:** High — student-visible rendering defects reached production without catch

---

### Model B — session-scoped override pattern

**ISSUE:** Needed to let a student study a non-sequential lesson (via the course picker) without permanently moving their progress pointer.
**SYMPTOM:** Without the pattern, the options were: (a) overwrite `current_lesson_code` permanently — student loses their sequence position; or (b) don't record completion/weak-areas for the picked lesson — data loss.
**ROOT CAUSE:** Architectural tension between "this session uses lesson X" and "the student's persistent progress position lives here." Both need to be true simultaneously.
**FIX:** `/session/start` accepts an optional `lessonCode`; validated against subject + SL/HL at the boundary; used for that session only. `isOverrideSession = session.lesson_code !== current_lesson_code`. In `message/route.ts`, completion and weak-area signals record against the picked lesson, but the sequence pointer never advances. `current_lesson_code` is untouched.
**PREVENTION:** Reusable pattern for any "do this once without losing my place" need. Three rules: (1) the override lives in the request/session, not in persistent state — pass it in, don't store it; (2) never write it back to the persistent pointer; (3) the persistence layer must actively guard with an explicit `isOverrideSession` check — not just "skip the write if the value is the same" (that's a different condition and a latent bug).
**CATEGORY:** Architecture / state management
**SEVERITY:** Medium — correctness; no student-visible bug on happy path but silent data corruption if pattern violated

---

*Master document, collated 16/05/2026; updated 21/05/2026 with the IB diagram-audit / CSS refactor / course_position session-opening harvest; updated 26/05/2026 with IB BM Layer 1 seed-generation build (Rules 22–26: evidence-before-encoding, verifier contradiction guards, generator prompt branch discipline, atomic str_replace, PDF-before-contradicting-verdict); updated 04/06/2026 with Econ + BM content-accuracy audit (Rule 27: no cross-subject content derivation by analogy; Curriculum: guide-verified audit findings, Tier 1–3 fixes, outstanding items); P2(g) placement fix verified live on Mia (Veralonia mock) — enforcement-at-point-of-action lesson logged as Rule 28; updated 04/06/2026 (same session) with the end-to-end loop audit — Layer 2 found disconnected from live marking (Rule 29), content_checklist proven valuable vs redundant band_descriptor, and a two-pointer lesson-sequencing ambiguity flagged (Signals); updated 04/06/2026 (same session) — Econ Layer 2 content_checklist wire-in COMPLETED and proven live (RPC returns scheme via LEFT JOIN LATERAL, system-prompt.ts injects it as a MARK SCHEME block, marking step quotes missed points verbatim; validated on served IB_ECON_093, Mia marked 2/4 and quoted the scheme's missed points verbatim on haiku with no manual paste). BM Layer 2 prompt shipped but blocked on the BM subject-key mismatch (Rule 30). Pointer/denormalised-name bug parked with workaround (set current_lesson_code + current_lesson_name together on any manual repoint). BM Layer 2 content_checklist now also LIVE and validated end-to-end (served IB_BM_004 describe question, scheme-anchored marking on production) after fixing the BM subject-key mismatch at the RPC layer (Option A translation); full subject-key normalisation logged as a scoped future project (B). Two-pointer ambiguity resolved: student_progress.current_lesson_code is authoritative, sessions.lesson_code is a snapshot. LAYER 2 NOW COMPLETE across all served scheme types: content_checklist (short-answer, both subjects) and hybrid (calculations) wired, validated live; scheme-bearing sessions routed to Sonnet for reliable method-mark reasoning (Rule 31); explicit "Mark: X out of N" forced to open every scheme-marked response. band_descriptor deliberately unwired (redundant with prompt markbands). The hybrid-generator scare resolved: only 2 served questions lacked a working scheme (pass-verified, just unpromoted) — no generator rebuild needed. Sources: every LC build/debug session (initial build through Sprint 8), the IB build/launch-prep sessions (02–16 May 2026), the IB build-hardening session of 21 May 2026 (`/admin/diagrams` rebuild, `.ib-session` extraction, `course_position` Layer 0 fix), and the IB BM Layer 1 build session of 26 May 2026 (87-candidate seed generation, 5-rule verifier patch, regen-rejected pipeline). Supersedes the standalone `BUILD_HARDENING.md` (IB) and `LC_BUILD_HARDENING.md`. Keep one copy in each Claude project. Add new entries in the standard format, tagged by product, ordered by category then severity; promote a lesson to TOP PREVENTION RULES only when it has cost real time more than once; updated 13/06/2026 with lessons from navigation + review build (signal stripping at read-back surface, stable cache keys not freeform LLM output, see-don't-infer on rendered UI, Model B session-scoped override pattern); updated 15/06/2026 with free-first signup build (preview webhook architecture, weak_area prefix scoping, destructive upsert pattern).*

---

## 15 June 2026 — Lessons from free-first signup build

### Stripe webhooks cannot reach ephemeral Vercel preview deployments

**ISSUE:** [IB] Payment completed on a preview branch deployment but `subscription_status` never flipped to `active`.
**SYMPTOM:** Sandbox checkout completed successfully (Stripe showed a paid session); `subscription_status` stayed `'inactive'`; free-tier gate never lifted on the preview deployment. Spent time investigating webhook secret selection and Vercel deployment protection before identifying the root cause.
**ROOT CAUSE:** Stripe fires each webhook to **one registered static URL** per endpoint record. A Vercel preview branch gets a new ephemeral URL on every push (e.g. `gradd-git-feat-free-first-signup-abc123.vercel.app`). Stripe has no knowledge of this URL — it fires to the URL registered in the Stripe dashboard (in this case the old `feat/ib-single-product` preview URL). The current preview deployment never receives the POST; `handleCheckoutComplete` never runs; status never flips. A second blocker can stack on top: if Vercel Preview Deployment Protection is re-enabled, Stripe's POST would receive a 401 even if the URL were correct.
**FIX:** Not a code bug. Three options for testing the convert path on preview: (a) `stripe listen --forward-to https://<current-preview-url>/api/webhooks/stripe` via the Stripe CLI — proxies sandbox events to the ephemeral URL without dashboard re-registration; (b) manually update the sandbox webhook endpoint in the Stripe dashboard to the current branch URL before each test run; (c) test convert on production (real card + refund) using the stable `gradd.ai` URL that the production webhook already targets.
**PREVENTION:** Before diagnosing "payment didn't activate on preview" as a code bug, rule out webhook delivery first: confirm (1) the registered sandbox webhook URL matches the current preview deployment URL, and (2) Vercel Preview Deployment Protection is disabled. If neither is true, the convert path cannot be tested on preview regardless of how correct the code is. The convert path code is not testable on arbitrary ephemeral preview URLs without one of the three options above. Validate convert end-to-end on production, not preview.
**CATEGORY:** Stripe/Billing
**SEVERITY:** High — wastes diagnostic time; convert path appears broken when it is not

---

### `weak_areas` subject scoping — prefix filter required at every read site

**ISSUE:** [IB] BM weak areas bled into IB Economics sessions — Mia referenced a charity/profit misconception (from a BM session) during an unrelated Econ lesson.
**SYMPTOM:** `session/message` route loaded all unresolved weak areas for the student and injected them into the Econ system prompt; IB Business content surfaced in IB Economics teaching. Dashboard showed "All clear" for both subjects despite one weak area existing in the DB — because the dashboard correctly filtered by `lesson_code` prefix and the BM weak area didn't match the Econ prefix.
**ROOT CAUSE:** `weak_areas` has **no `subject` column** — subject is implied by `lesson_code` prefix (`IB_ECON_` = Economics, `IB_BM_` = Business Management). Rule 3 ("filter by `student_id` AND `subject`") cannot be applied directly. Both `session/start` (L115–122) and `session/message` (L123) queried `weak_areas` with only `.eq('student_id', user.id).is('resolved_at', null)` — no prefix filter — so a bundle student's BM weak areas were always injected into their Econ sessions and vice versa. The dashboard had the correct fix already: `.like('lesson_code', '${lessonPrefix}%')` conditioned on a non-null prefix.
**FIX:** Derived `weakAreaPrefix` from `effectiveSubject` at both call sites (matching the dashboard's pattern at `dashboard/page.tsx:90–92`) and applied `.like('lesson_code', '${weakAreaPrefix}%')` conditionally — only when prefix is non-null, so LC sessions (null prefix) remain unfiltered. Commit `750753d`. Three call sites now agree: `session/start`, `session/message`, `dashboard/page.tsx`.
**PREVENTION:** This is the implicit-subject exception to Rule 3. When a table lacks a `subject` column and encodes subject in a code prefix, **every subject-scoped read must `.like('lesson_code', '<prefix>%')`**. Audit all query sites for the table when the pattern is introduced — one site having the filter while others don't produces the exact bleed seen here. The proper long-term fix is a real `subject` column on `weak_areas` (deferred); until then, the prefix filter at all three sites is the contract. If a new read site is added for `weak_areas`, it must carry the prefix filter or be explicitly justified as intentionally cross-subject.
**CATEGORY:** Database/Migration
**SEVERITY:** High — cross-subject content injection; teaching correctness defect

---

### Onboarding upsert is destructive — `upsert` ≠ insert-if-missing; audit flow ordering when a setup step moves

**ISSUE:** [IB] Free-first users who subscribed via "Go unlimited" were routed back through `/onboarding` post-checkout, which would have wiped their existing progress.
**SYMPTOM:** Did not bite in testing because the test user was on lesson 1 (the hardcoded default). Would have silently wiped `current_lesson_code`, `session_number`, `total_session_count`, and `course_position` back to starting values for any user past lesson 1.
**ROOT CAUSE:** Two stacked problems. (1) **`upsert` is not insert-if-missing.** `supabase.from('student_progress').upsert(row, { onConflict: 'student_id,subject' })` in `app/api/onboarding/ib/route.ts` (L85–120) OVERWRITES all columns when a matching row exists — including `current_lesson_code: 'IB_ECON_001'`, `session_number: 0`, `total_session_count: 0`. Any student past lesson 1 who re-triggers onboarding loses their place. (2) **Checkout `success_url` routes to `/onboarding`.** `app/api/checkout/ib/route.ts:80` sets `success_url: '/onboarding?subject=IB_BUNDLE&...'`. In the original pay-first flow this was correct (subscribe → onboard → start). Free-first inverts the ordering: onboarding already ran at signup; a post-payment redirect to `/onboarding` is now a destructive re-run. The LC product avoids this with a poller pattern: `success_url` → `/subscribe?success=true` → polls for `subscription_status === 'active'` → redirects to `/dashboard`. The dashboard is never re-onboarded.
**FIX (not yet applied — blocked):** (1) Make `/api/onboarding/ib` non-destructive: check for an existing row first (`.select('id').eq('student_id', ...).eq('subject', ...).single()`) and skip the upsert if one exists. INSERT for genuinely new students only. (2) Change IB `success_url` to the LC poller pattern: `/subscribe?success=true` (or equivalent IB poller route) — never `/onboarding`.
...
### Supabase SQL Editor migrations must be executed as ONE block (08/07/2026)
Running a multi-statement migration piecewise in the Supabase SQL Editor half-applied the AFM `acca_drills` schema migration: the `mode` column add was skipped while later statements landed, leaving the table in an inconsistent partial state. The migration's idempotent design (`ADD COLUMN IF NOT EXISTS`, guarded constraints, NULL-only backfill) let a clean re-run heal it with no damage — but it was the **verification queries** that surfaced the gap, not the apply.
**Prevention:** execute a migration as a SINGLE selected block (whole `begin;…commit;`), never statement-by-statement; and ALWAYS run the migration's verification queries afterward — piecewise/partial application is invisible without them. Idempotency is the safety net; the verification block is the detector.
**CATEGORY:** Schema/Migration · **SEVERITY:** Medium

### External reviewers re-litigate verified claims — house facts are override-by-default (08/07/2026)
Across two blog-post review cycles the external reviewer re-challenged already-verified facts twice each: the pass-rate house phrase ("consistently around 40% — among the lowest pass rates in the ACCA qualification" → wanted softened to "often sits around 40%") and the seven-year Strategic Professional rule (→ wanted vague "check the current rule"). Both had been verified against source; both reviewer "corrections" were regressions we correctly overrode.
**Prevention:** treat reviewer STYLE as accept-by-default but reviewer FACTS as re-check-by-default against our verification log before accepting. A verified house phrase is override-by-default — do not soften a sourced claim on a reviewer's unsourced preference. When overriding, record the override in the session bank so the next reviewer's identical challenge is answered from the log, not re-argued.
**CATEGORY:** Process/Review · **SEVERITY:** Low
