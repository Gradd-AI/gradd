...
> ⚠️ **CORRECTED 20/07/2026 — do not reinstate the old wording.** Earlier revisions of this file quoted L2 as *"Application and analysis — apply concepts, perform calculations, analyse, advise on application of techniques"* and L3 as *"Synthesis and evaluation — evaluate, recommend justified actions, assess trade-offs, exercise strategic judgement"*, presented as study-guide verbatim. **The em-dash tails appear NOWHERE in the guide** (each phrase returns 0 hits across all 21 pages). They were an editorial gloss in `scripts/afm-framework.ts` (`INTELLECTUAL_LEVELS`). **PURGED AT SOURCE 20/07/2026** — that constant now holds the guide-verbatim bare labels only, so the gloss can no longer be re-extracted. Blast-radius check: the gloss phrases returned **0 hits across all 133 `acca_drills` rows** (APM 91 + AFM 42, every served field) and were never consumed by any prompt builder or generator — latent, never served. The bare labels above are what ACCA actually publishes. See VERIFICATION LOG G1.
...
|---|---|---|---|
| **G1** | **FABRICATED GLOSS (the residue)** | §1a quoted L2 as "Application and analysis — **apply concepts, perform calculations, analyse, advise on application of techniques**" and L3 as "Synthesis and evaluation — **evaluate, recommend justified actions, assess trade-offs, exercise strategic judgement**", presented as study-guide verbatim. The guide gives **only the bare labels** on p.5. Each gloss phrase scores **0 hits across all 21 guide pages** ("perform calculations", "advise on application of techniques", "assess trade-offs", "exercise strategic judgement", "recommend justified actions"). Origin: `INTELLECTUAL_LEVELS` in `scripts/afm-framework.ts`, whose L1 sibling also carried the plainly editorial clause "not the primary level examined at AFM". | **FIXED + PURGED AT SOURCE** — §1a replaced with the guide's bare labels + the p.5 context sentence; the framework `INTELLECTUAL_LEVELS` gloss **removed** (now guide-verbatim bare labels), and the same gloss in `apm-framework.ts` purged too. **Blast radius (20/07/2026): 0 hits across 133 `acca_drills` rows** (APM 91 + AFM 42; fields question/context_text/model_answer/hint/full_reveal) and no consuming prompt builder or generator — the gloss was latent, never served, so no drill corrective round was required. |
...
**Upstream fix + blast-radius purge (20/07/2026).** The gloss was removed at source, not just annotated:
- `scripts/afm-framework.ts` `INTELLECTUAL_LEVELS` — gloss deleted; now holds the guide-verbatim bare labels. The file header's blanket "nothing here is invented or inferred" claim (which was false for this constant) was replaced with an explicit verbatim-scope statement.
- `scripts/apm-framework.ts` `INTELLECTUAL_LEVELS` — the **same defect** was confirmed (all 8 APM gloss phrases score 0 hits in the APM guide) and purged to bare labels, even though it was latent. Logged for a future APM trace in `AFM_SURFACED.md`.
- **Served-content sweep:** the gloss phrases were grepped across all authored/served content — `acca_drills` (133 rows, APM 91 + AFM 42; question/context_text/model_answer/hint/full_reveal), `lib/acca/tutor-personas.ts`, and every prompt builder / generator that reads intellectual level. **0 hits.** The only consumer of the `INTELLECTUAL_LEVELS` string constant was `scripts/test-apm-framework.ts` (asserts length > 10 — still passes on the bare labels). No drill or prompt ever carried the gloss, so no corrective round was needed.
