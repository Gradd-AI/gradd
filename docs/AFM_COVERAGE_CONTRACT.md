...
| Treasury function role (establishing/relocating; positive financial contribution) | E1a | 2 | **LIVE 24/07** — `55181aa8` EN1 (establishing/relocating) + `d0be009d` EN2 (positive financial contribution) |
| Derivatives-market operations (basis risk, margin, greeks) | E1b | 1 | — (**DEFERRED to exam-ready** — mechanical/definitional, no located E1b-specific examiner evidence; Step-0 ruling 2026-07-24) |
| Forex risk types (translation/transaction/economic) | E2a | 1 | **LIVE 24/07** — `f9f4f3d4` EN3 (F9 J16 p.5 fundamentals-tier evidence for the named-not-described mode; house-authored translation≠transaction disqualifier) |
...
## Mock exams (case-native content, NOT drills — RULED 2026-07-25)
- 1 mock = 1×50-mark Section A case (≥2 sections, all 4 PS skills) + 2×25-mark Section B questions (B and E always represented; each Section B question ≥2 of {Analysis & Evaluation, Scepticism, Commercial Acumen} — Communication is Section-A only per the syllabus's own §7 wording).
...
- **RULING (closes the phantom "generative engine" framing — see AFM_SURFACED phantom-spec finding, logged 2026-07-25): mocks are CASE-NATIVE AUTHORED CONTENT, not generated by any engine.** This is the proven, live pattern — APM's own mock (`acca_cases`/`acca_case_exhibits`/`acca_case_requirements`, `supabase/apm_questions/apm_mock_case_*.sql`) is hand-authored, and its own migration says why: *"Atomic drills each embed their own company and cannot be bolted together into one coherent case"* (`20260701120000_acca_cases.sql:15`) — a Section A case needs ONE shared company/dataset across ≥2 linked requirements, which neither pure assembly from published drills nor a from-scratch generative pipeline is needed to solve; hand-authoring against the same orchestration layer already does. **Marking is fully code-owned per requirement**, not authored/judged loosely: the deterministic calculator (npv/irr/apv/capm/duration/credit/valuation/international/risk/fxhedge/irhedge) owns every numeric figure and figure-vs-figure verdict for a calc-kind requirement, `narrative-marker.ts` (N1–N5) owns every discursive-mark verdict for a narrative-kind requirement, and `lib/acca/case-marking.ts` owns the professional-skills band→apportion conversion — exactly the same "code owns every figure/verdict" discipline as a standalone drill, just invoked per case requirement instead of per drill row. **Orchestration is reused as-is** — the live, paper-agnostic APM case load/turn/mark routes (`app/api/acca/case/{route,turn,mark,list}.ts`) + the mock clock (`app/api/acca/mock/route.ts`, `lib/acca/mocks.ts`) — paper-scoped by `paper_code`/`paper` filtering (2026-07-25 precondition fix), not rebuilt. **NO parallel/generative case engine is being built.**
...
| **VIABLE PAID LAUNCH** | B+E complete: calcs 1–12 + 8 narrative + 1 mock | ~56 drills + 1 mock (**57 drills shipped — all 12 calculators AND all 8 narrative drills now LIVE**; only the mock engine remains) | "Covers the sections guaranteed on every AFM exam" |
| **EXAM-READY** | Full A–E: 16 calcs + 22 narrative + 3 mocks | ~86 drills + 3 mocks | "Scratch to exam-ready for the AFM written paper" |

**Progress against tiers (24/07): B-SECTION-LIVE TIER CONTENT COMPLETE AND WALKED — all 10 calculators (calcs #1–#10) + the 5-drill B narrative cluster (pipeline #2) are LIVE, and both the calculator content (walked 18/07) and the narrative cluster (walked 21/07, frozen-layer PASS) have cleared their student walks. The tier-content-complete claim stands. AD SPEND is Grant-ruled (21/07): conditioned on the persona-hardening slot shipping + verifying before any spend — NOT unlocked by tier-content-complete alone. E-SECTION FULLY OPEN: calculator #11 (FX hedging) GATE-P-flipped LIVE 23/07, calculator #12 (IR hedging) GATE-P-flipped LIVE 24/07, the 3-drill E-narrative cluster (E1a×2 + E2a×1) GATE-P-flipped LIVE 24/07 (all three batches' student walks pending, non-blocking) — **ALL 12 of 12 calculators AND all 8 of 8 VIABLE-tier narrative drills are now shipped** (5 B + 3 E; E1b explicitly deferred to exam-ready, not silently dropped from the count). **Only the mock-rehearsal engine remains to close the VIABLE PAID LAUNCH tier.** Measured pace holds at ~1 batch per session.**
