...
- *Coached fix:* on a wrong calc, keep going — the linked discussion, recommendation, and downstream steps still score under the own-figure rule *where the own figure is subsequently used correctly* (OFR credit is conditional on correct downstream use, per the ACCA P2 Jun 2015 examiner report — not automatic). → **This is exactly where OFR / `carried` verdicts feed the coaching (Integration Notes #b).**
