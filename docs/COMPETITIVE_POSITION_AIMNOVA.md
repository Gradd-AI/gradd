...
## Their CHAT TUTOR vs Mia (the definitive head-to-head)
Aimnova also has a conversational AI tutor (separate from the rubric-marker). Tested it on the identical scarcity/shortage misconception used on Mia. Typed a fluent-but-wrong answer: "scarcity means there's a shortage... price rises until demand drops... that forces the choice."
Aimnova's tutor: (1) OPENED BY AFFIRMING the wrong answer — "That's a good point! You're connecting scarcity to price 🌟" (the fluency-is-not-completeness failure Mia was fixed against); (2) CORRECTED by restating the right definition over the top — "scarcity is more about X rather than Y" — and named the shortage/scarcity distinction generically, but NEVER diagnosed the faulty model (never said "you've collapsed scarcity into a shortage, that's why you reached for price"); (3) NO retrieval — handed over the answer, no re-test on the same point; (4) EXITED to a content link ("check the Exam Vault").
Verdict: their chat tutor is competent (better than their rubric-marker) but it CORRECTS + ROUTES TO CONTENT. Mia DIAGNOSES the specific faulty model + RE-TESTS until it lands (verified). The gap: correction vs diagnosis, told vs consolidated, affirms-fluent-wrong vs catches-it.
Honest caveat: their tutor is good enough that the difference is NOT obvious on a single question — it compounds over weeks (misconceptions Mia fixes don't recur; ones Aimnova corrects do). Our advantage is real but compounding, not first-session-obvious. Implication: the free-tier burn MUST demonstrate diagnosis-beats-correction; it cannot be claimed on a feature list.

## Their dashboard — the real product is a retention engine
Aimnova's dashboard reveals their actual product: a gamified progress-and-retention engine wrapping competent-but-shallow teaching. Features: streaks, XP/levels, daily challenges (+50 XP), achievements/badges (1/29), performance insights (accuracy %, streak), micro-topic + subject mastery bars, exam countdown ("149 days to exam"), interest-personalised explanations. PLUS a "Keep your parents in the loop — invite a parent to see your progress" loop — strategically sharp because the PARENT IS THE PAYER and this gives the buyer ongoing proof-of-value. They've optimised for HABIT and the FEELING of progress (what retains subscriptions + what parents see). Teaching is the content INSIDE the engagement machine.
Clean competitive trade, fully mapped:
- THEY win: engagement engine (streaks/XP/gamification), visible progress tracking, parent loop, breadth, price, polish.
- WE win: the actual teaching — diagnosis over correction (architecturally, the one thing they can't do).
SURFACE-LAYER IMPLICATION (sharpens earlier guidance): our highest-value surface investment is NOT more notes (don't race their content) — it's VISIBLE PROGRESS + a PARENT-VISIBILITY hook. Visible progress makes our invisible teaching advantage legible to a student before they're stuck; a parent loop serves OUR payer and retention (parent paying €15-20/mo needs to see the kid is learning). This beats matching their notes pages. Make the teaching's value visible — that's our core positioning problem, and progress-visibility solves it better than content-parity does.
