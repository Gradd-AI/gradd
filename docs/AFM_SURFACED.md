...
`required_point`, `model_answer`, `full_reveal`.

| class | measured | checkable | authoring burden |
|---|---|---|---|
| **1** figure repeated across ≥2 fields | **31 of 58 distinct** | yes, no domain knowledge | none |
| **2** explicit stated relation (`X, N% of Y`) | **1 in the whole batch** | yes — and it *was* the defect | declaration needed |
| **3** date anchors / stated intervals | **5 anchors, 8 intervals** | only against a declared timeline | real |
| **4** stated givens with no chain | the rest | **no** | n/a |

⚠️ **THE FINDING THAT DECIDES THIS. Class 1 goes GREEN on both of A3's hard errors.** Measured
against the pre-fix draft: **`33%` appeared exactly ONCE** (in `context_text` alone), so a
cross-field consistency check has nothing to compare it with. And **`eighteen months` appeared THREE
times — context, criteria and GOOD, all byte-consistent** — while the GOOD's use was the wrong one,
because it bound the interval to the wrong pair of anchors. **Consistency was not merely
insufficient; it was affirmatively green on the field that was wrong.**

**THE CALCULATORS CANNOT BE REUSED, AND THE REASON IS STRUCTURAL, NOT EFFORT.** The twelve AFM
families compute FROM raw inputs TO figures under the doctrine that code owns every number, and
`verifyNumericAnswer` compares a STUDENT's figure to an AUTHORED expected value. A narrative row has
no family, no raw inputs, no owned figures and no student — **the figures ARE the scenario**. There
is nothing to recompute against. Only the DISCIPLINE transfers, not the module. (Separately, the
`recompute` id → function registry is still not built and two facts block it — the lambdas are
closures over per-drill inputs, and `params` is `Record<string, number>` so it cannot carry
non-numeric discriminants. Even the case-row path cannot verify a STORED schema today.)

**SO THE ANALOGUE IS A DECLARATION, NOT A VERIFIER.** Class 2/3 works only if the author states the
relations that must hold — `{kind:'pct_of', part:94000, whole:280000, stated:34, tol:0.5}` and a
`timeline` of named events — and code recomputes them. That inverts the case-row design: there the
schema is GENERATED by a calculator that owns the numbers; here it is DECLARED by the author who
invented them. **It can only ever check what someone remembered to declare.**

**PRECEDENT ALREADY IN THE REPO:** `assertNarrativeNumbers` in
`scripts/authoring/build-sbl-crosswalk-ledger.ts` does exactly this for the ledger doc — every
narrative number must be DERIVED, ASSERTED with a written reason, or RHETORICAL. Its stated ceiling
is the one this would inherit, inverted: *number-WORDS only; a hand-typed DIGIT still gets through.*

**COST.** Class 1 ≈ `certainty-lint` (one pure lib module + fixture + runner; needs numeral
normalisation `1,400`/`1400`, currency-unit awareness, and a rounding tolerance), works retroactively
on all 19 live narrative rows with no backfill. Classes 2+3 need a schema addition, a gate, per-drill
authoring, and a backfill of 19 rows — **one ruling, not two**, since a timeline is just a declared
relation over dates.

**WHAT IT COULD NOT CATCH EVEN FULLY BUILT:** a figure consistent everywhere and wrong about the
world · a figure arithmetically right and wrong in the reasoning · **every P-N3 defect, which is not
numeric at all** · unit/currency errors unless the unit is declared · whether a rounding convention
is the intended one · anything the author did not declare. ⚠️ And note the interaction with the D7
ruling: the fix there was to **state the outcome as given and forbid the raw drivers**, which
