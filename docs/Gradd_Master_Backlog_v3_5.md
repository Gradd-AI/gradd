...
>
> **v3.6 — 12 June: NEXT TASK (ready to run) — fix BM weak_area_flag capture defect.**
> - DIAGNOSIS COMPLETE (verified): TEACH_BACK 1/3 = CORRECT by design (fires on 2nd miss of the SAME question; only Q1 was re-attempted — not a bug). WEAK_AREA_FLAG 0/3 = REAL DEFECT, BM-only.
> - ROOT CAUSE: the BM prompt (ib_business_tutor_system_prompt_v1_8.md) is MISSING the standalone `## WEAK AREA DETECTION` section that the Econ prompt HAS (ib_economics_tutor_system_prompt_v1_8.md lines 759-774). In BM, the RULE A/RULE B weak-area logic lives inside the SIGNAL PROTOCOL and is only pointed to from teaching-loop step 5 (BM line 371) — which is NOT active during MARKING turns. So during marking feedback, BM never fires weak_area_flag. Econ's standalone top-level section fires regardless of sub-protocol, so Econ is fine.
> - IMPACT: BM misconceptions are NOT being captured to session_events/weak_areas during marking → adaptive teaching for BM is under-capturing. Econ unaffected.
...
- [ ] **BUG: Diagram signal leak — raw `[DIAGRAM: DIAGRAM_DYNAMIC: ...]` text visible to student (15 Jun 2026).** When Mia wants a diagram the renderer can't produce (e.g. money-market/liquidity-preference, not in the 61-component library), she emits a `[DIAGRAM: DIAGRAM_DYNAMIC: <description>]` token that falls through to the student as visible plain text. Class: signal leak. Root cause: `getDiagramPath()` lookup returns null for unknown codes; the `DIAGRAM_DYNAMIC` format is not caught by the current signal-stripping regex path in `ChatInterface.tsx` before display. Fix options: (a) strip `DIAGRAM_DYNAMIC` like other signals and have Mia teach the diagram concept in prose when no component exists; (b) handle the dynamic format in the renderer. Found on free-first preview, HL Econ money-market lesson (15 Jun 2026).
